...
export class WebRTCClient {
   ...
   * @param {Object} [options.requestData] - Custom data sent to the backend with the offer
   */
  async connect(options = {}) {
    try {
      ...
      // Create data channel BEFORE creating the offer — the Pipecat
      // SmallWebRTC transport expects the client (offerer) to create it.
      this.dataChannel = this.peerConnection.createDataChannel('chat');

      this.dataChannel.onopen = () => {
        console.log('Data channel open');
        // Start sending keepalive pings — the server uses these to
      ...
      this.dataChannel.onmessage = (msgEvent) => {
        try {
          ...
          console.debug('Non-JSON data channel message:', msgEvent.data);
        }
      };
      ...
      if (options.requestData) {
        offerBody.request_data = options.requestData;
      }
