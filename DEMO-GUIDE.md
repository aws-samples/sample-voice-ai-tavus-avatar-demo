...
When a visitor shares their industry, connect voice agents to something concrete — then point them back to the agent or the reference architecture. The agent can go deeper on all of these.

| Industry | Example use case |
|---|---|
| **Financial Services** | Account servicing, wire transfers, fraud alerts — SOX/PCI compliant on AWS |
| **Healthcare** | Patient intake, clinical documentation, appointment scheduling — HIPAA eligible |
| **Insurance** | Claims intake (First Notice of Loss), policy servicing, renewal outreach |
| **Retail / QSR** | Drive-through ordering, customer support, returns and refunds |
| **Telecom** | Technical support diagnostics, plan changes, outage notifications |
| **Travel & Hospitality** | Reservations, disruption rebooking, multilingual concierge |
| **Government** | Citizen services, benefits enrollment, multilingual support — FedRAMP on GovCloud |
| **Energy & Utilities** | Outage reporting, billing support, surge call handling during weather events |
